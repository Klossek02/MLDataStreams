...
\begin{itemize}
    \item \textbf{Avazu:} We have extracted specific time details (hour, day, week) and created interaction features \new{(\texttt{site\_x\_hour}), computed as the concatenation of the hashed \texttt{site\_id} and the hour-of-day integer}. We have also used a sliding window of 1000 instances to calculate the recent CTR and interaction frequencies for sites and apps.
...
\subsection{\new{Proof-of-concept baseline: HAT in MOA}}

We first established a \new{proof-of-concept} baseline using the Hoeffding Adaptive Tree (HAT) -- an algorithm for streaming classification. \new{The HAT model was selected as the initial reference point because it includes built-in ADWIN-based drift adaptation. This makes a strong single-tree baseline for non-stationary streams.} The baseline HAT model has been evaluated on our processed Avazu and Criteo streams using the prequential procedure. \new{Moreover, the preliminary evaluation has been conducted in the MOA graphical interface and served to confirm that the data preprocessing has produced sensible results before proceeding to the Java-based experiments.}
...
\caption{\new{Agrawal dataset:} Error of the baseline HAT model over time, showing the adaptation gap after an abrupt concept drift. \new{Sudden drift takes place at instance 500,000.}}
...
\ForEach{incoming instance $(x_t,y_t)$ in chronological order}{
    ...
    \If{$D.\mathrm{detect}(e_t)$}{
        ...
        Notify the drift handler attached to $F$ and/or $M$\;
    }
...
it keeps 5 features. For the LED stream, it keeps 7 features, matching the
number of truly relevant LED-segment attributes. Real CTR and RBF experiments
...
\KwIn{Recent buffer $B$, top-$K$ size $K$, post-drift window length $W$, threshold $\tau$}
\KwOut{Updated selected feature set $S_t$, removed features $R$, added features $A$}
...
\If{$W$ post-drift instances have been collected}{
    ...
    Add features with $\Delta IG_i>\tau$ to $S_{base}$\;
    Remove features with $\Delta IG_i<-\tau$ from $S_{base}$\;
    If more than $K$ features remain, keep the $K$ highest by $IG_{after}$\;
    $R\leftarrow S_{old}\setminus S_{new}$ and $A\leftarrow S_{new}\setminus S_{old}$\;
    Rebuild the filtered header and notify the model adaptation listener\;
}
...
recovery rate 0.005. On LED, the subspace size is 7 so that a learner can in
principle cover all relevant LED-segment attributes. On the high-dimensional
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{../../results/plots/model_comparison_logloss.png}
...
\hline
Hubert Jaczy\'nski & \textbf{Analytics and final evaluation:} concept drift detection in raw data with ADWIN; final evaluation and comparative analysis of the proposed algorithm against baselines. \textbf{Report:} final evaluation and conclusion sections. \textbf{Presentation creation.} & $\sim$40h\\
\hline
Aleksandra K{\l}os & \textbf{Data engineering and baseline evaluation:} preprocessing Avazu and Criteo streams, exploratory data analysis, feature hashing, missing-value imputation, baseline evaluation in MOA, and scripts for Log Loss and adaptation-gap analysis. \textbf{Report:} introduction, data preparation, and evaluation methodology sections. \textbf{Presentation creation.} & $\sim$40h \\
\hline
Jakub Oganowski & \textbf{Algorithm implementation:} Java implementation of the SRP/OFS integration, drift-aware feature selection, subspace adaptation, and ensemble weighting. \textbf{Report:} architecture of the proposed solution section. \textbf{Presentation creation.} & $\sim$40h \\
\hline
\end{tabular}%
}
\end{table}
...
Table~\ref{tab:changes} maps each reviewer remark to the section(s) modified in
response.
...
&
Sect.~4.1, Algorithm~\ref{alg:main_pipeline}; Sect.~4.3,
...
&
Sect.~4.3 and Algorithm~\ref{alg:main_pipeline}. \\
\hline
...
&
Sect.~2.3, Sect.~3.1, Sect.~4.2, figure captions. \\
\hline
...
&
Sect.~2.4, Table~\ref{tab:datasets_summary}, Sect.~5.1, Sect.~5.4. \\
\hline
\textit{Comment whether drift detected in Sect. 3.1 is the only drift present;
...
&
Sect.~3.1, Sect.~4.3, Fig.~\ref{fig:eval_drift_counts}. \\
\hline
...
summary.
&
Sect.~4.2, Table~\ref{tab:real_final_metrics}, Sect.~5.1. \\
\hline
...
&
Table~\ref{tab:real_final_metrics}; \texttt{results\_summary.csv}. \\
\hline
...
&
Fig.~\ref{fig:model_comparison_logloss_all}, Sect.~5.4. \\
\hline
