...
through a unified \texttt{StreamProvider} interface. Model wrappers expose a
...
5,000-instance comparison window and a 0.005 Information Gain change
threshold; Agrawal experiments use a smaller 800-instance window and a
threshold of 0.05, reflecting the larger and more abrupt swings induced by
...
feature drift instead of being fixed at construction time. Second, members
whose subspaces are reconstructed do not vote at full strength immediately:
the ensemble keeps track of how much fresh evidence each member has seen and
weighs its vote accordingly during recovery. Both mechanisms are designed to
...
\texttt{SubspaceManager} owns the per-learner feature subsets and exposes an
...
to neutral features when the strengthened pool is too small. The fraction of
the original subspace that had to be replaced is then compared against a
reset ratio: subspaces that lost more than this fraction are flagged for full
reinitialisation, since the surviving learner state would be inconsistent
with its new feature set.

The \texttt{WeightManager} mirrors the reset decision on the voting side.
Members reported by the subspace manager as reset have their weight reduced
...
amount of new data they have processed. With a recovery rate of 0.001 per
instance, full recovery takes roughly 700 instances, which is comparable to
...
\begin{enumerate}
    ...
    \item Learners whose subspace changed beyond the reset ratio are
    reinitialised on the new sub-header.
    \item The \texttt{WeightManager} reduces the vote weight of reset learners
    and gradually restores it during the subsequent training instances.
\end{enumerate}
...
On real CTR datasets, DASRP uses 10 base Hoeffding-tree learners, subspaces
of size 20, ADWIN with $\delta=0.002$, reset weight 0.3, recovery rate
0.001, and a reset ratio of 0.5. On Agrawal, the configuration uses subspaces
of size 4, ADWIN with $\delta=0.001$, reset weight 0.5, and recovery rate
0.005. The different settings reflect the much smaller feature space and
sharper drift dynamics of the synthetic stream.
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figures/evaluation/eval_real_efficiency_tradeoff.png}
\caption{Quality/runtime trade-off on real CTR streams. Lower and further left
is better.}
\label{fig:eval_efficiency_tradeoff}
\end{figure}
